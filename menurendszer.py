...
class EtteremRendszer:
            ...
            with open('recept.csv', 'r', encoding='utf-8') as f:
                ...
                for sor in olvaso:
                    if not sor or len(sor) < 3:
                        continue
                    etel_nev = sor[0].strip()
                    alapanyag = sor[1].strip()
                    if not etel_nev or not alapanyag:
                        continue
                    try:
                        menny = float(sor[2])
    ...
    def rendeles_felvetel(self, asztal_szam, etel_nev):
        ...
        if not self.rendelheto_e(etel):
            ...
            for alapanyag in h:
                if not elso:
                    uzenet += ", "
                uzenet += alapanyag + " (hiányzik " + format(h[alapanyag], ".3f") + ")" #A format egy beépített függvény, amivel egy értéket (például egy számot) alakíthatsz meghatározott formájú szöveggé
                elso = False
            return uzenet
    ...
    def etel_hozzaad(self, nev, ar, hozzavalok):
        ...
        """
        if self.keres_etel_kulcs(nev) is not None:
